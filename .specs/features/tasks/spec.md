...
Tasks and their sessions stored under `.iara/tasks/`:
...
- Sessions move from `.iara/sessions/` to `.iara/tasks/<task-id>/sessions/`
...
- **"default branch"** entry always present — shows actual default branch name detected from the repos (e.g. `● main`, `● master`, `● develop`). Launches Claude in original repos (no worktree). Sessions stored in `.iara/tasks/default/sessions/`. Also serves as backward compat with existing `.iara/sessions/`.
- "+ New Task" at top — launches Claude with `/iara:new-task`
...
- `enter` on "+ New Task" → launch Claude with `/iara:new-task`
...
**Why `.worktrees/` at project root (not inside `.iara/`):**
...
- Clean separation: `.iara/` = metadata, `.worktrees/` = working copies
...
### R5: `/iara:new-task` Skill (replaces `/iara:new-intention`)

**Only available when launched in a worktree context** (not from default branch).

Adaptation of `/iara:new-intention` for task creation:
...
- **Current**: auto-launch `/iara:new-intention`
...
- `/iara:new-intention` is deleted; project setup becomes a simpler skill that only handles metadata (no branching)
...
```go
// Before:
session.Save(projectDir, s)     // hardcoded .iara/sessions/
...
- Default branch: `<project>/.iara/tasks/default/sessions/`
- Worktree task: `<project>/.iara/tasks/<task-id>/sessions/`
...
- Default branch gets its own dev config at `.iara/tasks/default/dev-config.json`
...
- `iara internal save-task '<json>'` — save task metadata to disk
- `iara internal finish-task` — mark task completed, remove worktrees

### R12: Migration / Backward Compatibility

- The **"default branch"** entry in the task list serves as backward compat
- Existing sessions in `.iara/sessions/` are shown under the "default branch" entry
- On first use, existing `.iara/sessions/` are migrated to `.iara/tasks/default/sessions/` (or symlinked)
